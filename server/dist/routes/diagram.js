...
router.get('/full', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const findUser = (name) => {
            ...
            return allUsers.find(u => (u.username && u.username.toLowerCase().trim() === cleanName) ||
        ...
        const teamLeader = await prisma.diagramTeamLeader.create({
            data: {
                ...
                managerLinks: managerIds && managerIds.length > 0 ? {
                    create: managerIds.map((managerId, index) => ({
                        managerId,
                        isPrimary: index === 0
                    }))
                } : undefined
    ...
    try {
        ...
        if (teamLeaderIds && teamLeaderIds.length > 0) {
            await prisma.managerTeamLeaderLink.createMany({
                data: teamLeaderIds.map((teamLeaderId, index) => ({
                    managerId,
                    teamLeaderId,
                    isPrimary: index === 0
                }))
...
router.post('/assignments/bulk', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const assignments = await Promise.all(mailerIds.map((mailerId, index) => prisma.diagramMailerAssignment.create({
            data: {
                mailerId,
                teamId,
                isPrimary: index === 0
