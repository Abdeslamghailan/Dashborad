...
function generateIPsFromCIDR(cidr, serverName) {
...
router.get('/:entityId', authenticateToken, async (req, res) => {
    try {
...
// Update proxy server
router.put('/:entityId/:proxyId', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'ADMIN' && req.user?.role !== 'MAILER') {
        return res.status(403).json({ error: 'Access denied' });
    }
    try {
...
// Toggle proxy status
router.patch('/:entityId/:proxyId/status', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'ADMIN' && req.user?.role !== 'MAILER') {
        return res.status(403).json({ error: 'Access denied' });
    }
    try {
...
// Delete proxy server
router.delete('/:entityId/:proxyId', authenticateToken, async (req, res) => {
    if (req.user?.role !== 'ADMIN' && req.user?.role !== 'MAILER') {
        return res.status(403).json({ error: 'Access denied' });
    }
    try {
