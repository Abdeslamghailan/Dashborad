...
import { authenticateToken, requireAdmin, requireAdminOrMailer } from '../middleware/auth.js';
import { getEntityHistory, getHistoryByType, getRecentChanges } from '../services/historyService.js';
import prisma from '../db.js';
const router = Router();
// Get history for a specific entity
router.get('/entity/:entityId', authenticateToken, requireAdminOrMailer, async (req, res) => {
    try {
...
router.get('/type/:entityType', authenticateToken, requireAdminOrMailer, async (req, res) => {
    try {
...
router.get('/recent', authenticateToken, requireAdminOrMailer, async (req, res) => {
    try {
...
router.get('/', authenticateToken, requireAdminOrMailer, async (req, res) => {
    try {
        const { entityId, entityType, username, changeType, startDate, endDate, limit = '100' } = req.query;
        // Build where clause
        const where = {};
        if (entityId)
        ...
        let start = startDate ? new Date(startDate) : threeMonthsAgo;
