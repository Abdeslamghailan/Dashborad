 ...
 */
export async function logChange(req, params, explicitActor) {
    try {
        let actorId;
        let actorUsername;
        let actorRole;
 ...
 */
export async function getEntityHistory(entityId, limit = 5) {
    try {
 ...
 */
export async function getHistoryByType(entityType, limit = 5) {
    try {
        const history = await prisma.changeHistory.findMany({
            where: { entityType },
            orderBy: { createdAt: 'desc' },
 ...
 */
export async function cleanupOldHistory(keepPerEntity = 50) {
    try {
