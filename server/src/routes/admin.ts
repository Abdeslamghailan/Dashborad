...
router.put('/users/:id/approve', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
...
router.post('/revoke', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    const { userId, entityId } = req.body;
