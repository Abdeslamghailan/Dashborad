...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      {/* Left Navigation */}
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col shrink-0 transition-all duration-300 z-20">
        {/* Logo Section */}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-gray-50/50">
        {/* Header */}
        <header className="h-16 bg-white border-b border-gray-200 sticky top-0 z-30 flex items-center justify-between px-8 shadow-sm/50 backdrop-blur-sm bg-white/90">
          ...
          <div className="flex items-center gap-6">
            ...
            {user?.photoUrl ? (
              <img
                src={user.photoUrl}
                alt={user.username || 'User'}
                className="w-9 h-9 rounded-full shadow-md shadow-indigo-200 ring-2 ring-white object-cover"
              />
            ) : (
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-indigo-200 ring-2 ring-white">
                {user?.username?.[0]?.toUpperCase() || 'U'}
              </div>
            )}
          </div>
        ...
        {/* Scrollable Content */}
        <main className={`flex-1 overflow-y-auto scroll-smooth ${location.pathname === '/dashboard-reporting' ? 'p-2' : location.pathname === '/tools' ? 'p-0' : 'p-8'}`}>
